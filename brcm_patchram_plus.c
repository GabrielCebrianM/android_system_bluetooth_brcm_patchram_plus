...
uchar hci_write_sleep_mode[] = { 0x01, 0x27, 0xfc, 0x0c,
	0x01, 0x00, 0x00, 0x01/*bt_wakeup*/, 0x01/*host_wakeup*/, 0x01, 0x01, 0x00, 0x00, 0x00,
	0x00, 0x00 };

uchar hci_write_sleep_mode_rk903[] = { 0x01, 0x27, 0xfc, 0x0c, 
...
extern int bt_get_chipname(char* name, int len);

int get_hcd_by_chip(char* hcdfile)
{
    if (!strncmp(bt_chip, "rk903_26M", strlen("rk903_26M")))
        strcpy(hcdfile, "/system/etc/bluez/rk903_26M.hcd");
    else if (!strncmp(bt_chip, "rk903", strlen("rk903")))
        strcpy(hcdfile, "/system/etc/bluez/rk903.hcd");
    else if (!strncmp(bt_chip, "bcm4329", strlen("bcm4329")))
        strcpy(hcdfile, "/bcm4329_samsung.hcd");
    else if (!strncmp(bt_chip, "mv8787", strlen("mv8787")))
        strcpy(hcdfile, "/system/etc/bluez/mv8787.hcd");
    else
    {
        fprintf(stderr, "Not support BT chip, please specify the firmware\n");
        return -2;
    }
...
parse_patchram(char *optarg)
{
    ...
    char hcdfile[1024];
    if (!strcmp(optarg, "bychip"))
    {
        int ret = 0;
        printf("Get hcd file by BT chip!\n");
        ret = get_hcd_by_chip(hcdfile);
        if (ret<0)
            exit(6);
        optarg = hcdfile;
...
parse_cmd_line(int argc, char **argv)
{
	...
	PFI parse[] = { parse_patchram, parse_baudrate,
		...
		parse_scopcm, parse_i2s, parse_no2bytes, parse_tosleep, parse_readextra};
...
proc_patchram()
{
    ...
    if (!strncmp(bt_chip, "bcm4329", strlen("bcm4329"))) {
    	if (!no2bytes) {
...
#ifdef READEXTRA
    if(readextra) {
        read(uart_fd, buffer, 1);  //cz@rock-chips.com: 20110701: receive an unexpected character 0x00
    }
#endif
...
proc_enable_lpm()
{
    if (!strncmp(bt_chip, "bcm4329", strlen("bcm4329")))
        ...
        hci_send_cmd(hci_write_sleep_mode_rk903, sizeof(hci_write_sleep_mode_rk903));
...
proc_enable_hci()
{
    ...
    if (!strncmp(bt_chip, "rk903", strlen("rk903")))
        proto = HCI_UART_LL;
...
read_default_bdaddr()
{
    ...
    (bd_addr[5] == 0))
    {
        ...
        printf("There is no bd_addr written in the ID block!");
    ...
    ba2str((bdaddr_t *)bd_addr, bd_addr_str);
    printf("bd_addr: %s\n",bd_addr_str);

    printf("Read default bdaddr of %s\n", bd_addr_str);
    parse_bdaddr(bd_addr_str);
...
main (int argc, char **argv)
{
    ...
    {
        fprintf(stderr, "Can't read BT chip name\n");
        exit(3);
    }
    
    printf("BT chip name is: %s\n", bt_chip);
